...
public class DatePicker extends WheelPicker {
    ...
    private boolean maxIsCurrentDate = false;
    ...
    public DatePicker(Activity activity, boolean maxIsCurrentDate) {
        this(activity, YEAR_MONTH_DAY, maxIsCurrentDate);
    }
     ...
     */
    public DatePicker(Activity activity, @Mode int mode, boolean maxIsCurrentDate) {
        super(activity);
        this.mode = mode;
...
//        }
        ...
        for (int i = getCurrentYear() - 3; i <= (maxIsCurrentDate ? getCurrentYear() : getCurrentYear() + 3); i++) {
            years.add(String.valueOf(i));
        }
        createMonth(12);
        for (int i = 1; i <= 31; i++) {
    ...
    public void setCurrentDate() {
        setSelectedItem(getCurrentYear(), getCurrentMonth(), getCurrentDay());
    }
    ...
    private void createMonth(int maxMonth) {
        if (months.size() > 0)
            months.clear();
